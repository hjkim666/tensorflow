...
W4 = tf.Variable(tf.random_normal([4 * 4 * 128, 512], stddev=0.01))
L4 = tf.reshape(L3, [-1, 4 * 4 * 128])
L4 = tf.matmul(L4, W4)
L4 = tf.nn.relu(L4)
L4 = tf.nn.dropout(L4, keep_prob)

W5 = tf.Variable(tf.random_normal([512, 128], stddev=0.01))
L5 = tf.matmul(L4, W5)
L5 = tf.nn.relu(L5)

W6 = tf.Variable(tf.random_normal([128, 4], stddev=0.01))
hypothesis = tf.matmul(L5, W6)
